...
"""
    ...
    js_replacements = {
        ...
        'if (result.score.appleBonus) scoreParts.push(`사과 +${result.score.appleBonus}`);': 'if (result.score.appleBonus) scoreParts.push(`りんご +${result.score.appleBonus}`);',
        '`${ruleText}${result.word} 배치 완료. ${scoreParts.join(" · ")}`': '`${ruleText}${result.word} を置きました。${scoreParts.join(" · ")}`',
        'return normalizeMode(mode) === "snake" ? "스네이크" : "일반";': 'return normalizeMode(mode) === "snake" ? "スネーク" : "通常";',
        ...
        'const mode = result.mode === "snake" ? " 스네이크" : "";': 'const mode = result.mode === "snake" ? " スネーク" : "";',
        'return `${result.boardSize}x${result.boardSize}${mode} 한국어 ${finish} ${result.score.toLocaleString("ko-KR")}점`;': 'return `${result.boardSize}x${result.boardSize}${mode} 日本語 ${finish} ${result.score.toLocaleString("ja-JP")}点`;',
        ...
        'setShareStatus(copied ? "X 공유 창을 열고 이미지를 복사했습니다." : "X 공유 창을 열었습니다.")': 'setShareStatus(copied ? "Xの共有画面を開き、画像をコピーしました。" : "Xの共有画面を開きました。")',
        '.catch(() => setShareStatus("X 공유 창을 열었습니다."));': '.catch(() => setShareStatus("Xの共有画面を開きました。"));',
        'setShareStatus("공유했습니다.");': 'setShareStatus("共有しました。");',
        '`${result.filled} / ${result.total}칸 · ${result.turns}개 단어`': '`${result.filled} / ${result.total}マス · ${result.turns}単語`',
        'setShareStatus("결과 이미지를 클립보드에 복사했습니다.");': 'setShareStatus("結果画像をクリップボードにコピーしました。");',
