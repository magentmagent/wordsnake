...
"""
    ...
    js_replacements = {
        ...
        'if (result.score.appleBonus) scoreParts.push(`사과 +${result.score.appleBonus}`);': 'if (result.score.appleBonus) scoreParts.push(`apples +${result.score.appleBonus}`);',
        '`${ruleText}${result.word} 배치 완료. ${scoreParts.join(" · ")}`': '`${ruleText}${result.word} placed. ${scoreParts.join(" · ")}`',
        'return normalizeMode(mode) === "snake" ? "스네이크" : "일반";': 'return normalizeMode(mode) === "snake" ? "Snake" : "Classic";',
        ...
        'const mode = result.mode === "snake" ? " 스네이크" : "";': 'const mode = result.mode === "snake" ? " Snake" : "";',
        'return `${result.boardSize}x${result.boardSize}${mode} 한국어 ${finish} ${result.score.toLocaleString("ko-KR")}점`;': 'return `${result.boardSize}x${result.boardSize}${mode} English ${finish} ${result.score.toLocaleString("en-US")} pts`;',
        ...
        'setShareStatus(copied ? "X 공유 창을 열고 이미지를 복사했습니다." : "X 공유 창을 열었습니다.")': 'setShareStatus(copied ? "Opened X share window and copied the image." : "Opened X share window.")',
        '.catch(() => setShareStatus("X 공유 창을 열었습니다."));': '.catch(() => setShareStatus("Opened X share window."));',
        'setShareStatus("공유했습니다.");': 'setShareStatus("Shared.");',
        '`${result.filled} / ${result.total}칸 · ${result.turns}개 단어`': '`${result.filled} / ${result.total} tiles · ${result.turns} words`',
        'setShareStatus("결과 이미지를 클립보드에 복사했습니다.");': 'setShareStatus("Result image copied to clipboard.");',
